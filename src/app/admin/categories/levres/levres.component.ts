...
@Component({
  selector: 'app-levres',
  templateUrl: './levres.component.html',
  styleUrls: ['./levres.component.css']
})
export class LevresComponent {
  constructor(
  ...
  UpdateData() {
    for (let i = 0; i < this.Producatdata.length; i++) {
      if (this.Producatdata[i].productfield == 'levres') {
...
.subscribe(
  (data)=> (this.Producatdata = this.Producatdata.filter((p:any)=>p.id != id))
);
